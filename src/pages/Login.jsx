    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 400, animation: 'fadeIn 0.3s ease' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <div style={{ fontSize: 11, color: 'var(--text3)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 8 }}>
            Scanplore.ia
          </div>
          <div style={{ fontSize: 36, fontWeight: 600, color: 'var(--accent)', letterSpacing: '-0.03em' }}>
            Bussola
          </div>
          <div style={{ fontSize: 13, color: 'var(--text3)', marginTop: 8 }}>
            Gestao comercial inteligente
          </div>
        </div>
        ...
        <div className="card" style={{ padding: 32 }}>
            ...
            {['login', 'cadastro'].map(m => (
              <button key={m} onClick={() => { setModo(m); setErro(''); setSucesso('') }}
                style={{
                  ...
                  background: modo === m ? 'var(--accent-dim)' : 'transparent',
                  color: modo === m ? 'var(--accent)' : 'var(--text3)',
                  border: modo === m ? '1px solid rgba(0,200,150,0.3)' : '1px solid var(--border)',
          ...
          <form onSubmit={entrar}>
            ...
            <button type="submit" className="btn btn-primary" disabled={loading}
              ...
              {loading ? 'Aguarde...' : modo === 'login' ? 'Entrar no Bussola' : 'Criar minha conta'}
            </button>
          </form>
        </div>
